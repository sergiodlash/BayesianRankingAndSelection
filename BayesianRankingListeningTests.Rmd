...
\
The R code for this example can be found [HERE](https://github.com/sergiodlash/BayesianRankingAndSelection), with the Stan model and the RMarkdown file.\
...
The idea behind an *indifference-zone* is that, for example, ratings that differ less that 5 points on a MUSHRA scale are can be considered as basically the same. So we can focus only on differences that are larger than this thresholds. The section on *indifference-zones* of [Petteri Hyvärinen's post](https://petterihyvarinen.fi/blog/posts/bayesian-mushra/) explains it best.
